...
// Check GCC
	...
	#define modmask Integer(63)
	#define IntegerBitSize 64
#else 
	#define Integer uint32_t 
	#define modmask Integer(31)
	#define IntegerBitSize 32
#endif
...
#define IntegerMaskTable s_maskTable32
	...
	static void setBitEnv(Integer i, Integer integer)
	{
		integer |= Integer(1) << i;
	}

	static void resetBitEnv(Integer i, Integer integer)
	{
		integer &= ~(Integer(1) << i);
	}

	static bool isBitSetEnv(Integer i, Integer integer)
	{
		return integer & (Integer(1) << i);
	}
	...
	*/
	...
	/**
	Description: 	-- DEPRECATED -- Use "getBlockEnv" instead!! Returns the array's values [i, ... , i + length - 1] in a 64 bit integer.
	...
	*/
	...
	/**
	Description: 	-- DEPRECATED -- Use "getBlockEnv" instead!! Returns the array's values [i, ... , i + length - 1] in a 32 bit integer.
	Parameter:		i 		- The first bit of the desired block.
	length 	- The block length in bits.
	array	- The array in which the blocks are stored.
	Preconditions:	The array has to be at least the size of i * tau + tau size in bits.
	length has to be lesser then or equal to 32.
	Postconditions: --
	Result:			Returns the value of the i-th block, the bits [i, ... ,i + length - 1].
	Complexity: 	This function calculates in O(1) time and with O(1) bits of space the i-th block.
	*/
	...
	/**
	Description: 	-- DEPRECATED -- Use "setBlockEnv" instead!! Sets the "array's" values [i, ... , i + length - 1] to "block".
	Parameter:		i 		- The first bit of the desired block.
	...
	*/
	...
	array	- The array in which the blocks are stored.
	Preconditions:	The array has to be at least the size of i + tau - 1 size in bits.
	length has to be less then or equal to 32.
	Postconditions: The bits [i, ... , i + length - 1] are equal to the "block" parameter.
	Result:			--
	Complexity: 	This function sets in O(1) time and with O(1) space the bits [i, ... , i + tau - 1] in array.
	*/
	...
	/*--DEPRECATED -- Use "setBlockEnv" instead!!*/
